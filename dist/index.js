...
async function apiRequest(path, body) {
    ...
    if (!res.ok) {
        ...
        throw new Error(`BidCraft API error ${res.status}: ${err.error || res.statusText}`);
    }
    return res.json();
}
...
// ── Tool 1: Analyze RFP ──────────────────────────────────────────────
server.tool("analyze_rfp", "Analyze RFP (Request for Proposal) text and extract structured requirements — mandatory, desirable, and informational items with deadlines and sections. Useful for understanding what an RFP is asking for before writing a response.", {
    text: z
        .string()
        .describe("The full RFP text or relevant sections to analyze. Include requirements, deadlines, evaluation criteria, and scope of work."),
}, async ({ text }) => {
    ...
    try {
        ...
        const reqList = (data.requirements || [])
            .map((r, i) => `${i + 1}. [${r.type.toUpperCase()}] ${r.section}: ${r.description}${r.deadline ? ` (Due: ${r.deadline})` : ""}`)
            .join("\n");
        return {
            content: [
                {
...
server.tool("generate_proposal", "Generate an AI-powered proposal response from RFP text. Returns executive summary, key differentiators, and compliance preview. Use after analyzing the RFP to create a compelling bid response.", {
    rfpText: z
        .string()
        .describe("The full RFP text or key sections to generate a proposal from. Include requirements, evaluation criteria, and deliverables."),
    companyContext: z
        .string()
        .optional()
        .describe("Optional context about your company — capabilities, past performance, team size, certifications — to personalize the proposal."),
    ...
    try {
        const body = { quickMode: true, rfpText };
        if (companyContext)
            body.companyContext = companyContext;
        const data = await apiRequest("/api/generate", body);
...
server.tool("check_compliance", "Cross-reference proposal content against RFP requirements to identify coverage gaps. Shows which requirements are addressed, partially covered, or missing from the proposal.", {
    rfpText: z
        .string()
        .describe("The original RFP text with requirements to check against."),
    proposalText: z
        .string()
        .describe("The proposal or response text to evaluate for compliance with the RFP requirements."),
    ...
    try {
        ...
        const results = requirements.map((req) => {
            const keywords = req.description
                ...
                .split(/\s+/)
                .filter((w) => w.length > 4);
            const matchCount = keywords.filter((k) => proposalLower.includes(k)).length;
            const coverage = keywords.length > 0 ? matchCount / keywords.length : 0;
            const status = coverage > 0.5 ? "COVERED" : coverage > 0.2 ? "PARTIAL" : "MISSING";
        ...
        const score = Math.round(((covered + partial * 0.5) / results.length) * 100);
        const details = results
            .map((r) => `[${r.status}] ${r.section}: ${r.description} (${r.coverage}% keyword match)`)
            .join("\n");
        return {
            content: [
                {
...
server.tool("bidcraft_info", "Get information about BidCraft capabilities, pricing plans, and features for AI-powered RFP response and proposal generation.", {}, async () => {
    return {
        content: [
            {
                type: "text",
